...
export default class PartidoDAO {
  ...
  async gravar(partido) {
    if (partido instanceof Partido) {
      const conexao = await conectar();
      const sql = `INSERT INTO partido (nome, sigla, registro) VALUES (?, ?, ?)`;
      const parametros = [partido.nome, partido.sigla, partido.registro];
  ...
  async alterar(partido) {
    if (partido instanceof Partido) {
      ...
      const parametros = [partido.nome, partido.registro, partido.sigla];
      await conexao.execute(sql, parametros);
      await global.poolConexoes.releaseConnection(conexao);
    }
  }
